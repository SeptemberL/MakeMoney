# 自选分时均价与压力/支撑（watchlist-intraday-metrics）

本文档由变更 `watchlist-intraday-avg-pressure-support`（2026-03-21 归档）合并入主规格库。

## Requirements

### Requirement: 腾讯批量行情接口暴露均价与压力/支撑

`POST /api/quotes/tencent` 在 `success: true` 且返回 `quotes` 时，对每一条成功解析的报价对象 SHALL 包含字段 `avg`、`pressure_line`、`support_line`（JSON 中无效时为 `null`），语义如下：
...
- `pressure_line`：压力位，当 `avg>0` 时为 `avg / K`，否则为 `null`；`K` MUST 等于 `0.98848`。
- `support_line`：支撑位，当 `avg>0` 时为 `avg × K`，否则为 `null`。
